...
void inspect_readyToEat(FILE *fptr, int curr_date[]) {
    int i=0;
    boolean found=FALSE;
    while(i<aisle_index && !found ) {
        if(aisle_array[i]!=NULL && strcmp(aisle_array[i]->product_type, "readyToEat")==0) {
            found=TRUE;
        }
        else i++;
    } 
    if(found) {
        ...
        fprintf(fptr, "Ready-To-Eat items not found!\n\n");
    }
}
...
void inspect_dairyProduct(FILE *fptr, int curr_date[]) {
    ...
    boolean found=FALSE;
    while(i<aisle_index && !found ) {
        if(aisle_array[i]!=NULL && strcmp(aisle_array[i]->product_type, "dairyproduct")==0) {
            found=TRUE;
        }
        else i++;
    } 
    if(found) {
        ...
        fprintf(fptr, "Dairy Products not found!\n\n");
    }
}
...
int main() {
    ...
    FILE *fptr3=fopen("../Input_Folder/Purchase_input.txt", "r");
    FILE *fptr4=fopen("../Output_Folder/Bill_output.txt", "w");
